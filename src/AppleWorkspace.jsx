...
function ApplicationsPage({ companies, intelligence, selectedId, selectCompany, openAdd, openNotifications, navigate, openNode, openQuickUpdate }) {
  ...
  return <div className="aw-page aw-applications-page">
    <PageHeader title="秋招投递总表" subtitle="公司、岗位、批次、状态、进度和下一步集中在一张表里。" action="添加投递" onAction={openAdd}><TimelineAction onClick={() => openNode(pageItems.some((company)=>company.id===selectedId)?selectedId:pageItems[0]?.id)} /><IconButton label="通知" onClick={openNotifications}><BellSimple /></IconButton></PageHeader>
    <div className="aw-lifecycle-tabs" role="group" aria-label="投递生命周期筛选">{[["active","活跃"],["archived","已结束"],["all","全部"]].map(([id,label])=><button type="button" key={id} className={lifecycleFilter===id?"is-active":""} onClick={()=>setLifecycleFilter(id)}>{label}<b>{id==="all"?applications.length:applications.filter((company)=>(id==="archived")===isArchivedApplication(company)).length}</b></button>)}</div>
    <div className="aw-toolbar aw-tracker-toolbar"><SearchField value={localQuery} onChange={setLocalQuery} placeholder="搜索公司、岗位、城市或批次" /><label>阶段<select value={stageFilter} onChange={(event) => setStageFilter(event.target.value)}><option value="all">全部</option>{STAGES.map((stage) => <option key={stage.id} value={stage.id}>{stage.label}（{counts[stage.id] || 0}）</option>)}</select></label><label>赛道<select value={trackFilter} onChange={(event)=>setTrackFilter(event.target.value)}><option value="all">全部</option>{[...TRACK_OPTIONS,"未分类"].map((value)=><option key={value}>{value}</option>)}</select></label><label>渠道<select value={channelFilter} onChange={(event)=>setChannelFilter(event.target.value)}><option value="all">全部</option>{[...CHANNEL_OPTIONS,"未分类"].map((value)=><option key={value}>{value}</option>)}</select></label><label>重视程度<select value={priorityFilter} onChange={(event) => setPriorityFilter(event.target.value)}><option value="all">全部</option>{[5,4,3,2,1].map((value) => <option key={value} value={value}>{value} 分</option>)}</select></label><span className="aw-tracker-count">显示 {filtered.length} / {applications.length} 条</span></div>
    <Panel className="aw-table-panel aw-tracker-table-panel"><div className="aw-table aw-app-table aw-tracker-table"><div className="aw-tr aw-th"><span>公司 / 岗位</span><span>城市</span><span>批次 / 来源</span><span>投递时间</span><span>当前状态</span><span>当前进度</span><span>下一步行动 / DDL</span><span>重视程度</span><span>资料</span><span>职位链接</span></div>{pageItems.map((company) => { const next = nextActionSummary(company); return <div className={`aw-tr ${company.id === selectedId ? "is-selected" : ""}`} key={company.id}><button type="button" className="aw-company-cell aw-table-company-link" onClick={() => navigate("roles", company.id)}><CompanyLogo company={company} size="sm" /><span><strong>{company.name}</strong><small>{company.role}</small></span></button><span><MapPin />{company.location || "未注明"}</span><span><strong>{categoryLabel(company.track)}</strong><small>{categoryLabel(company.channel)} · {company.batch || "批次未注明"}</small></span><span>{shortDate(applicationDate(company))}</span><span><b>{statusLabel(company)}</b></span><span>{currentProgress(company)}</span><button type="button" className="aw-table-action-link" onClick={()=>openQuickUpdate(company.id)}><strong>{next.label}</strong><small>{next.date ? `${shortDate(next.date)} ${next.time}` : "点击快速更新"}</small></button><span><PriorityPill value={company.priority} /></span><span className="aw-material-flags"><small className={company.jd||company.jdImage?"is-ready":""}>JD</small><small className={company.notes?"is-ready":""}>备注</small></span><span>{company.jobUrl ? <a className="aw-job-link" href={company.jobUrl} target="_blank" rel="noreferrer"><LinkSimple />打开职位</a> : <small>待补充</small>}</span></div>; })}{!filtered.length && <EmptyState title="没有符合条件的投递" text="调整搜索或筛选条件，也可以添加一条新记录。" action="添加投递" onAction={openAdd} />}</div><footer className="aw-table-footer"><span>{filtered.length?`显示 ${pageStart+1}–${Math.min(pageStart+APPLICATIONS_PAGE_SIZE,filtered.length)}，共 ${filtered.length} 条`:"暂无记录"}</span>{pageCount>1?<nav className="aw-pagination" aria-label="投递记录分页"><button type="button" aria-label="上一页" disabled={currentPage===1} onClick={()=>setPage((value)=>Math.max(1,value-1))}><CaretLeft/></button><strong aria-live="polite">第 {currentPage} / {pageCount} 页</strong><button type="button" aria-label="下一页" disabled={currentPage===pageCount} onClick={()=>setPage((value)=>Math.min(pageCount,value+1))}><CaretRight/></button></nav>:<span className="aw-table-hint">点击公司查看完整资料；点击下一步快速更新</span>}</footer></Panel>
  </div>;
}
...
function RolesPage({ companies, intelligence, selectedId, selectedCompany, selectCompany, openAdd, openNotifications, openRoleEditor, openNode, openQuickUpdate }) {
  const [localQuery, setLocalQuery] = useState("");
...
function SchedulePage({ companies, intelligence, openNode, openNotifications, selectCompany, selectedId, openQuickUpdate }) {
  ...
  useEffect(() => { const validIds=new Set(companies.map((company)=>company.id));const newIds=companies.filter((company)=>!knownCompanyIdsRef.current.has(company.id)).map((company)=>company.id);knownCompanyIdsRef.current=validIds;setVisibleIds((current)=>[...new Set([...current.filter((id)=>validIds.has(id)),...newIds])]); }, [companies]);
  const days = useMemo(() => Array.from({ length: GANTT_WINDOW_DAYS }, (_, index) => addDays(ganttStart, index)), [ganttStart]);
  const cells = useMemo(() => buildCalendar(month, companies), [month, companies]);
  const counts = countStages(companies, intelligence);
  const timelineCompanies = companies.filter((company) => visibleIds.includes(company.id));
  const ganttPageCount = Math.max(1,Math.ceil(timelineCompanies.length/GANTT_ROLE_PREVIEW_SIZE));
  const currentGanttPage = Math.min(ganttPage,ganttPageCount);
  const displayedTimelineCompanies = timelineCompanies.slice((currentGanttPage-1)*GANTT_ROLE_PREVIEW_SIZE,currentGanttPage*GANTT_ROLE_PREVIEW_SIZE);
  const timelineIndex = useMemo(() => new Map(companies.map((company)=>{const byDate=new Map();scheduleNodes(company).forEach((node)=>{const nodes=byDate.get(node.date)||[];nodes.push(node);byDate.set(node.date,nodes);});byDate.forEach((nodes)=>nodes.sort((a,b)=>String(a.time||"").localeCompare(String(b.time||""))));return [company.id,byDate];})),[companies]);
  ...
  useEffect(()=>{const index=timelineCompanies.findIndex((company)=>company.id===selectedId);if(index>=0)setGanttPage(Math.floor(index/GANTT_ROLE_PREVIEW_SIZE)+1);else setGanttPage((current)=>Math.min(current,ganttPageCount));},[selectedId,visibleIds,companies,ganttPageCount]);
  ...
  return <div className="aw-page aw-schedule-page">
    ...
    {mode === "timeline" ? <>
      <section className="aw-timeline-filter-panel"><button type="button" className="aw-timeline-filter-summary" aria-expanded={filtersExpanded} aria-controls="aw-timeline-filter-options" onClick={()=>setFiltersExpanded((value)=>!value)}><span><FunnelSimple/>显示岗位</span><small>{visibleIds.length} / {companies.length} 个岗位已显示</small><CaretDown/></button>{filtersExpanded&&<div id="aw-timeline-filter-options" className="aw-timeline-filter-options"><div className="aw-timeline-filter-actions"><button type="button" onClick={()=>setVisibleIds(companies.map((company)=>company.id))}>全选</button><button type="button" onClick={()=>setVisibleIds([])}>清空</button></div><div className="aw-timeline-filters">{companies.map((company) => <label key={company.id} className={visibleIds.includes(company.id)?"is-active":""}><input type="checkbox" checked={visibleIds.includes(company.id)} onChange={()=>setVisibleIds((current)=>current.includes(company.id)?current.filter((id)=>id!==company.id):[...current,company.id])}/><CompanyLogo company={company} size="sm" /><small>{company.name} · {company.role}</small><Check /></label>)}</div></div>}</section>
      ...
      <Panel className="aw-calendar-main"><div className="aw-weekdays">{["周日","周一","周二","周三","周四","周五","周六"].map((day)=><span key={day}>{day}</span>)}</div><div className="aw-month-grid">{cells.map((cell)=><button key={cell.key} aria-pressed={selectedCalendarDate===cell.key} aria-label={`${shortDate(cell.key)}，${cell.events.length} 项安排，点击查看全部`} className={`${cell.current?"":"is-muted"} ${cell.key===dateKey(new Date())?"is-today":""} ${selectedCalendarDate===cell.key?"is-selected-day":""}`} onClick={()=>setSelectedCalendarDate(cell.key)}><span>{cell.day.getDate()}</span>{cell.events.slice(0,3).map(({company,node},index)=><em className={`event-${stageFor(company,intelligence)}`} key={`${company.id}-${node.id||index}`}><b>{node.time?`${node.time} · ${nodeName(node)}`:nodeName(node)}</b><small>{company.name}</small></em>)}{cell.events.length>3&&<small className="aw-calendar-overflow">+{cell.events.length-3} 更多</small>}</button>)}</div></Panel>
      <aside className="aw-right-stack"><Panel title="当日日程" subtitle={shortDate(selectedCalendarDate)} action={<TimelineAction label="添加" onClick={()=>openNode(calendarAgendaEvents[0]?.company.id||selectedId||companies[0]?.id,selectedCalendarDate)}/>}>{calendarAgendaEvents.length?<><div id="aw-calendar-agenda" className={`aw-upcoming-list aw-calendar-agenda-list ${calendarAgendaExpanded?"is-expanded":""}`}>{displayedCalendarEvents.map(({company,node},index)=><button key={`${company.id}-${node.id||index}`} onClick={()=>node.isAction?openQuickUpdate(company.id):openNode(company.id,node.date,node)}><span className="aw-soft-icon"><CalendarBlank/></span><span><strong>{nodeName(node)}</strong><small>{company.name} · {company.role}</small></span><em>{node.isAction?"行动 DDL":node.time||"全天"}</em></button>)}</div>{calendarAgendaEvents.length>8&&<button type="button" className="aw-agenda-more" aria-expanded={calendarAgendaExpanded} aria-controls="aw-calendar-agenda" onClick={()=>setCalendarAgendaExpanded((value)=>!value)}>{calendarAgendaExpanded?"收起当日日程":`查看其余 ${calendarAgendaEvents.length-8} 项`}<CaretDown/></button>}</>:<EmptyState icon={CalendarBlank} title="这天还没有安排" text="选择其他日期，或添加一个新的时间节点。"/>}</Panel><Panel title="阶段概览"><div className="aw-summary-list">{STAGES.slice(1).map((stage)=><div key={stage.id}><span><i className={`tone-${stage.color}`}/>{stage.label}</span><b>{counts[stage.id]}</b></div>)}</div></Panel><Panel title="图例"><div className="aw-legend-grid"><span><i className="tone-green"/>面试</span><span><i className="tone-orange"/>Offer</span><span><i className="tone-purple"/>测评</span><span><i className="tone-red"/>结束</span></div></Panel></aside>
    </div>}
  </div>;
}
...
export function AppleWorkspace() {
  ...
  useEffect(()=>{if(!hydrated||dirtyVersion===0)return;try{localStorage.setItem(draftKey,JSON.stringify({version:1,updatedAt:new Date().toISOString(),baseUpdatedAt:revisionRef.current,profile,companies}));}catch{setSaveState({status:"error",message:"浏览器草稿空间不足，请保持页面打开直到磁盘保存完成"});}},[companies,profile,hydrated,dirtyVersion,draftKey]);
  useEffect(()=>{if(!hydrated||!writeEnabled||dirtyVersion===0)return;clearTimeout(persistTimer.current);const snapshot={dirtyVersion,profile,companies};persistTimer.current=setTimeout(()=>{const run=async(task)=>{const queue=persistQueueRef.current;if(queue.inFlight){queue.pending=task;return;}queue.inFlight=true;let failed=false;setSaveState({status:"saving",message:"正在写入本机数据"});try{const payload={version:1,updatedAt:new Date().toISOString(),baseUpdatedAt:revisionRef.current,profile:task.profile,companies:task.companies};const response=await fetch("/api/workspace",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});const value=await response.json().catch(()=>({}));if(!response.ok){const error=new Error(value.error||"保存接口不可用");error.isConflict=response.status===409;throw error;}revisionRef.current=value.savedAt||revisionRef.current;setStorage(value.storage||storage);if(dirtyVersionRef.current===task.dirtyVersion&&!queue.pending){localStorage.removeItem(draftKey);dirtyVersionRef.current=0;setDirtyVersion(0);setSaveState({status:"saved",message:`已保存，现有 ${value.storage?.backupCount??storage?.backupCount??0} 份历史备份`});}else{setSaveState({status:"saving",message:"检测到新的修改，继续保存"});}}catch(error){failed=true;queue.pending=null;setWriteEnabled(false);setConflict(error.isConflict?"conflict":"error");setSaveState({status:"error",message:error.isConflict?"另一个页面已保存新版本，本页草稿仍保留":error.message||"保存失败，本页草稿仍保留"});}finally{queue.inFlight=false;const pending=queue.pending;queue.pending=null;if(pending&&!failed)run(pending);}};run(snapshot);},320);return()=>clearTimeout(persistTimer.current);},[companies,profile,hydrated,writeEnabled,dirtyVersion,draftKey]);
  ...
  const downloadCurrentDraft=()=>{const draft=conflict==="stale"&&staleDraft?staleDraft:{version:1,updatedAt:new Date().toISOString(),baseUpdatedAt:revisionRef.current,profile,companies};downloadBlob(new Blob([JSON.stringify(draft,null,2)],{type:"application/json"}),`秋招工作台-本页未保存草稿-${dateKey(new Date())}.json`);flash("本页草稿已下载");};
  const applyRemoteWorkspace=(value)=>{setCompanies(value.companies);setProfile(value.profile&&typeof value.profile==="object"?{...EMPTY_PROFILE,...value.profile}:EMPTY_PROFILE);setSelectedId(value.companies?.[0]?.id||"");revisionRef.current=value.updatedAt||null;dirtyVersionRef.current=0;setDirtyVersion(0);setStorage(value.storage||null);setWriteEnabled(true);localStorage.removeItem(draftKey);if(staleDraft?.storageKey)localStorage.removeItem(staleDraft.storageKey);setStaleDraft(null);setSaveState({status:"saved",message:"已载入磁盘中的最新版本"});};
  ...
  const undoLast=()=>{if(!undoState)return;setCompanies(undoState.companies);setSelectedId(undoState.selectedId);setDirtyVersion((value)=>value+1);clearTimeout(undoTimerRef.current);setUndoState(null);flash("已撤销刚才的操作");};
  ...
  const openNode=(companyId,date,node=null,options={})=>{const targetCompanyId=companyId||selectedId||companies[0]?.id;if(!targetCompanyId){flash("请先添加一个岗位");return;}setNodeDraft({companyId:targetCompanyId,originalCompanyId:targetCompanyId,date:date||dateKey(new Date()),id:node?.id||"",type:node?.type||"自定义",title:node?.title||"",time:node?.time||"",note:node?.note||"",review:node?.review||null,syncActionDeadline:Boolean(options.syncActionDeadline)});setModal("node");};
  ...
  const saveProfile=(event)=>{event.preventDefault();const data=new FormData(event.currentTarget);setProfile({name:String(data.get("name")||"").trim(),title:String(data.get("title")||"").trim(),location:String(data.get("location")||"").trim()});setDirtyVersion((value)=>value+1);setModal(null);flash("个人资料已保存");};
  const saveCompany=(event)=>{event.preventDefault();const data=new FormData(event.currentTarget);const name=String(data.get("name")||"").trim();const role=String(data.get("role")||"").trim();if(!name||!role)return;const rawAppliedAt=String(data.get("appliedAt")||"").trim();const appliedAt=expandMonthDay(rawAppliedAt);if(rawAppliedAt&&!appliedAt){flash("投递时间无效，请按月-日填写");return;}const logo=logoDraft?.dataUrl||matchLogo(name);const timeline=appliedAt?[{id:`node-${Date.now()}`,date:appliedAt,type:"投递",title:"",time:"",note:""}]:[];let status=String(data.get("status")||"待投递");let progress=String(data.get("progress")||"未开始");if(appliedAt&&status==="待投递")status="已投递";if(appliedAt&&progress==="未开始")progress="已投递";const company={id:`position-${Date.now()}`,name,team:String(data.get("team")||name).trim(),role,track:String(data.get("track")||"").trim(),channel:String(data.get("channel")||"").trim(),location:String(data.get("location")||"").trim(),batch:String(data.get("batch")||"").trim(),appliedAt,status,progress,nextAction:String(data.get("nextAction")||"").trim(),nextActionDeadline:String(data.get("nextActionDeadline")||""),priority:normalizePriority(data.get("priority")),jobUrl:String(data.get("jobUrl")||"").trim(),mark:name.slice(0,1),logoUrl:logo,jd:String(data.get("jd")||"").trim(),jdImage:jdImageDraft?.dataUrl||"",notes:String(data.get("notes")||"").trim(),timeline,files:[]};setCompanies(current=>[...current,company]);setDirtyVersion((value)=>value+1);setSelectedId(company.id);setLogoDraft(null);setJdImageDraft(null);setModal(null);flash("投递记录已添加");};
  const saveRole=(event)=>{event.preventDefault();const data=new FormData(event.currentTarget);const rawAppliedAt=String(data.get("appliedAt")||"").trim();const appliedAt=expandMonthDay(rawAppliedAt,applicationDate(selected));if(rawAppliedAt&&!appliedAt){flash("投递时间无效，请按月-日填写");return;}setCompanies(current=>current.map(company=>{if(company.id!==selectedId)return company;let timeline=[...(company.timeline||[])];const applicationIndex=timeline.findIndex((node)=>/投递|网申/.test(`${node.type||""}${node.title||""}`));if(appliedAt&&applicationIndex>=0)timeline[applicationIndex]={...timeline[applicationIndex],date:appliedAt};else if(appliedAt)timeline.push({id:`node-${Date.now()}`,date:appliedAt,type:"投递",title:"",time:"",note:""});else if(!rawAppliedAt&&applicationIndex>=0)timeline.splice(applicationIndex,1);return {...company,name:String(data.get("name")||company.name).trim(),role:String(data.get("role")||company.role).trim(),team:String(data.get("team")||"").trim(),track:String(data.get("track")||"").trim(),channel:String(data.get("channel")||"").trim(),location:String(data.get("location")||"").trim(),batch:String(data.get("batch")||"").trim(),appliedAt,status:String(data.get("status")||"待投递"),progress:String(data.get("progress")||"未开始"),nextAction:String(data.get("nextAction")||"").trim(),nextActionDeadline:String(data.get("nextActionDeadline")||""),priority:normalizePriority(data.get("priority")),jobUrl:String(data.get("jobUrl")||"").trim(),jd:String(data.get("jd")||"").trim(),jdImage:jdImageDraft?.dataUrl||"",notes:String(data.get("notes")||"").trim(),timeline};}));setDirtyVersion((value)=>value+1);setJdImageDraft(null);setModal(null);flash("岗位资料已保存");};
  const saveQuickUpdate=(event)=>{event.preventDefault();const data=new FormData(event.currentTarget);setCompanies(current=>current.map(company=>{if(company.id!==selectedId)return company;const next={...company,status:String(data.get("status")||company.status),progress:String(data.get("progress")||company.progress),nextAction:String(data.get("nextAction")||"").trim(),nextActionDeadline:String(data.get("nextActionDeadline")||""),priority:normalizePriority(data.get("priority")),lastActivityAt:dateKey(new Date())};if(isArchivedApplication(next)&&data.get("clearAction")){next.nextAction="";next.nextActionDeadline="";}return next;}));setDirtyVersion((value)=>value+1);setModal(null);flash("岗位状态与下一步已更新");};
  const completeAction=(companyId)=>{const previous=companies;setCompanies(current=>current.map(company=>company.id===companyId?{...company,nextAction:"",nextActionDeadline:"",lastActivityAt:dateKey(new Date())}:company));setDirtyVersion((value)=>value+1);beginUndo("行动已完成",previous,selectedId);};
  ...
  const saveNode=(event)=>{event.preventDefault();const data=new FormData(event.currentTarget);const review={questions:String(data.get("reviewQuestions")||"").trim(),answer:String(data.get("reviewAnswer")||"").trim(),weakness:String(data.get("reviewWeakness")||"").trim(),next:String(data.get("reviewNext")||"").trim()};const node={id:nodeDraft.id||`node-${Date.now()}`,date:nodeDraft.date,type:nodeDraft.type,title:String(data.get("title")||"").trim(),time:String(data.get("time")||"").trim(),note:String(data.get("note")||"").trim(),...(Object.values(review).some(Boolean)?{review}: {})};setCompanies(current=>{const next=upsertTimelineNode(current,nodeDraft.companyId,nodeDraft.originalCompanyId,node);return nodeDraft.syncActionDeadline?next.map((company)=>company.id===nodeDraft.companyId?{...company,nextActionDeadline:node.date}:company):next;});setDirtyVersion((value)=>value+1);setModal(null);flash("时间节点已保存，首页、甘特图、日历和分析已同步");};
  ...
  return <div className="aw-app">
    <aside className="aw-sidebar">
      ...
      <div className="aw-sidebar-bottom">
        <button className={`aw-save-indicator state-${staleDraft?"error":saveState.status}`} onClick={()=>staleDraft?setConflict("stale"):saveState.status==="error"?setConflict(conflict||"error"):setView("settings")}><span>{staleDraft||saveState.status === "error" ? <X/> : <Check/>}</span><p><strong>{staleDraft?"有冲突草稿":saveState.status === "saving" ? "正在保存" : saveState.status === "error" ? "保存失败" : "本机已保存"}</strong><small>{storage?.backupCount ?? 0} 份历史备份</small></p><CaretRight/></button>
        <button className={view==="settings"?"is-active":""} onClick={()=>setView("settings")}><Gear/><span>数据与设置</span></button>
    ...
    {modal==="role"&&selected&&<Modal title={`${selected.name} · ${selected.role}`} onClose={()=>setModal(null)}><form className="aw-form aw-tracker-form" onSubmit={saveRole}>
      ...
      <div><label>招聘批次<input name="batch" list="batch-options" defaultValue={selected.batch}/></label><label>投递时间（月-日）<input name="appliedAt" inputMode="numeric" pattern="[0-1][0-9]-[0-3][0-9]" defaultValue={monthDayInput(applicationDate(selected))} placeholder="07-21"/></label></div>
